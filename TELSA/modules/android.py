...
def havoc(bot: Bot, update: Update):
    ...
    if device == '':
        reply_text = "TYPE YOUR DEVICE CODENAME INTO IT\nFOR Eg, `/havoc tissot`"
    ...
    if fetch.status_code == 200:
        ...
        keyboard = [[InlineKeyboardButton(text="📥DOWNLOAD📥", url=f"{url}")]]
...
def evo(bot: Bot, update: Update):
    ...
    if device == 'gsi':
        reply_text = """
Please check Evolution Updates channel! @TELSABOTS or Click the button down below to download the GSIs!
"""
        keyboard = [[InlineKeyboardButton(text="📥DOWNLOAD📥", url="https://sourceforge.net/projects/evolution-x/files/GSI/")]]
        message.reply_text(reply_text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode=ParseMode.MARKDOWN, disable_web_page_preview=True)
        return
